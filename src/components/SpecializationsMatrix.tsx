...
import { Box, Search, ShieldCheck, Sparkles, Brain, Cpu, CheckCircle2 } from 'lucide-react';
...
export const SPECIALIZATIONS_DATA: PluginSpec[] = [
  ...
  { id: 'bricks', name: 'Bricks', category: 'builders', version: '1.11+', type: 'Visual Builder', description: 'Native structure inspection, nestable elements, and Bricks global color/typography tokens.', codeSnippet: `\\Bricks\\Database::get_data($post_id, 'bricks_data');` },
  ...
  { id: 'kadence', name: 'Kadence', category: 'themes', version: '1.2+', type: 'Core Theme', description: 'Kadence Hooked Elements, conditional display rules, and global typography tokens.', codeSnippet: `\\Kadence\\get_theme_option('palette');` },
  ...
  // Field Plugins (6)
  { id: 'acf', name: 'ACF', category: 'fields', version: '6.3+', type: 'Custom Fields', description: 'Field groups, Flexible Content, Repeaters, Options Pages, and ACF Blocks.', codeSnippet: `get_field_objects($post_id);` },
  { id: 'jetengine', name: 'JetEngine', category: 'fields', version: '3.5+', type: 'Custom Fields', description: 'Custom Post Types, Taxonomies, Relations, Query Builder, and Listing Grids.', codeSnippet: `jet_engine()->cpt->data->get_items();` },
  ...
  { id: 'acpt', name: 'ACPT', category: 'fields', version: '1.1+', type: 'Custom Fields', description: 'GraphQL support, Schema.org generators, and visual relationships.', codeSnippet: `\\ACPT\\Models\\PostTypeModel::get_all();` },
  { id: 'pods', name: 'Pods', category: 'fields', version: '3.2+', type: 'Custom Fields', description: 'Pods CPT relationships, extendable tables, and field templates.', codeSnippet: `pods('project', $id)->field('budget');` },
  { id: 'ase', name: 'ASE', category: 'fields', version: '7.4+', type: 'Site Utilities', description: 'Admin and Site Enhancements: custom body/admin classes, post type orders, and clean URLs.', codeSnippet: `\\ASE\\Options::get_enabled_modules();` },
  ...
  { id: 'rankmath', name: 'Rank Math', category: 'seo', version: '1.0.230+', type: 'SEO Suite', description: 'Schema Generator (JSON-LD), Content AI score, and redirection rules.', codeSnippet: `\\RankMath\\Helper::get_post_meta('title', $post_id);` },
  { id: 'aioseo', name: 'All in One SEO', category: 'seo', version: '4.7+', type: 'SEO Suite', description: 'Smart tags, TruSEO analysis, and local business rich snippets.', codeSnippet: `aioseo()->meta->title->getTitle($post);` },
...
export const SpecializationsMatrix: React.FC = () => {
  ...
  const categories = [
    { id: 'all', label: 'All Specializations (38)', count: 38 },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-2xl border border-border bg-card shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-indigo-600 via-blue-600 to-purple-600 flex items-center justify-center text-white shadow-md">
              <Sparkles className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-bold text-foreground">Specializations: 38 Builders & Plugins Covered End-to-End</h2>
                <span className="hidden sm:inline-block px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-primary/10 text-primary uppercase">
                  · and growing
                </span>
              </div>
              <p className="text-xs text-muted-foreground">
                Adds deep architectural specializations to your AI (Antigravity, Cursor, Claude Code) with persistent memory across sessions.
              </p>
            </div>
          </div>

          {/* System Requirements Badge */}
          <div className="p-3 rounded-xl border border-border bg-muted/40 font-mono text-[11px] space-y-1 sm:text-right flex-shrink-0">
            <div className="text-muted-foreground font-semibold">REQUIREMENTS:</div>
            <div className="text-primary font-bold">Novamira Free · WordPress 6.9+ · PHP 8.0+</div>
          </div>
        </div>
        ...
        <div className="space-y-3 pt-2">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Filter by name (e.g. Elementor, Bricks, ACF, JetEngine, Rank Math)..."
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[560px] overflow-y-auto pr-1">
          {filtered.map((plugin) => (
            <button
              key={plugin.id}
            ...
            >
              ...
              <div className="pt-2 border-t border-border/50 flex items-center justify-between text-[10px] font-mono text-muted-foreground">
                ...
                <span className="text-emerald-500 font-semibold flex items-center gap-1">
                  <ShieldCheck className="w-3 h-3" /> Specialization Ready
                </span>
              </div>
            </button>
          ))}
        ...
        <div className="rounded-2xl border border-border bg-card p-5 space-y-4 shadow-sm flex flex-col justify-between">
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs font-mono text-muted-foreground">PHP 8.0+ / WP 6.9+</span>
            </div>
            ...
            <div className="space-y-1.5 pt-2">
              <span className="text-xs font-mono font-semibold text-foreground">Specialized AI Runtime Hook:</span>
            ...
            <div className="p-3 rounded-xl bg-primary/5 border border-primary/20 space-y-1">
              ...
              <p className="text-[11px] text-muted-foreground">
                Schema changes and hook decisions for {activePlugin.name} are permanently saved in AGENT_MEMORY.md.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-primary text-primary-foreground text-xs font-semibold hover:opacity-90 transition-opacity flex items-center justify-center gap-1.5 shadow-sm"
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span>Copy Specialized PHP Hook</span>
          </button>
        </div>
      </div>
    </div>
